...
- Use async/await APIs over closure-based variants whenever they exist
...
- Strongly prefer not to use `ObservableObject`, `@Published`, `@StateObject`, `@ObservedObject`, or `@EnvironmentObject` unless they are unavoidable
...
- Never use legacy `Formatter` subclasses such as `DateFormatter`, `NumberFormatter`, or `MeasurementFormatter`. Always use the modern `FormatStyle` API instead
...
- Use the newest ScrollView APIs for item scrolling and positioning; avoid older scrollView APIs like ScrollViewReader
...
- If the project uses Localizable.xcstrings, prefer to add user-facing strings using symbol keys with `extractionState` set to "manual", accessing them via generated symbols

## Skills Available
This project has specialized skills for different aspects of Swift development:
- **SwiftData skill**: Use for data persistence, predicates, CloudKit considerations, and indexing opportunities
- **SwiftTesting skill**: Use for unit and integration tests (not UI tests), async tests, and modern testing patterns
- **SwiftUI skill**: Use for comprehensive view reviews, accessibility compliance, and performance optimization
